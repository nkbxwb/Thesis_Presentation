...
event data, often in the face of censoring.
\end{block}
Common survival estimates and tests
\begin{itemize}
 \item The survival function $S(t)=P(T>t)$ is estimated by the Kaplan-Meier Curve
 $$\hat{S}(t)=\prod_{t_i<t}\frac{n_i -d_i}{n_i}$$
Where $n_i$ is the number of survivors minus the number of censored cases just before time $t_i$
and $d_i$ is the number of deaths at time $t_i$, and $n_i$ is the risk set
\end{itemize}
\end{frame}

\begin{frame}{Cox Regression}
 \begin{itemize}
  \item The log rank test compares two survival curves to see if from the same distribution
 ...
 by $$\lambda(t)=\lim_{\Delta t \rightarrow 0+}\frac{P[t\leq T<t+\Delta t|t\leq T]}{\Delta t}$$
   \item Cox regression models hazard by $$h(t|Z)=h_0(t)exp(\sum_{k=1}^{p}\beta_k)$$
 \end{itemize}
