...
\begin{frame}{How to generate imputations}
\begin{itemize}
...
event data, often in the face of censoring (not knowing the exact time of failure).
\end{block}
Examples:
\begin{itemize}
 \item The survival of patients after a liver transplant in a hospital
 \begin{itemize}
  \item Censoring: study ending, patients die before study starts, subject moves away
 \end{itemize}

 \item The time until a child learns a new task
 \begin{itemize}
  \item Censoring: Refuse participation, move away, don't recall the exact time they learned,
...
\begin{itemize}
 \item The survival function $S(t)=P(T>t)$ is estimated by the nonparametric Kaplan-Meier Curve
 $$\hat{S}(t)=\prod_{t_i<t}\frac{n_i -d_i}{n_i}$$
Where $n_i$ is the number of subject in the risk set at time $t_i$
and $d_i$ is the number of deaths at time $t_i$
\end{itemize}
\end{frame}
...
\begin{frame}{Cox Regression}
 ...
 by $$\lambda(t)=\lim_{\Delta t \rightarrow 0+}\frac{P[t\leq T<t+\Delta t|t\leq T]}{\Delta t}$$
   \item Cox regression models hazard by $$h(t|Z)=h_{0}(t)\exp(\sum_{k=1}^{p}\beta_{k}Z_{k})$$
   \item Where $h_{0}(t)$ is the baseline hazard
   \item $\beta_k$ is found by partial maximum likelihood estimation
 \end{itemize}
